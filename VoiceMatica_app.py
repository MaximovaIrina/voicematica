...
from VoiceMaticaRu import VoiceMaticaRu
...
if LANGUAGE == "ru":
    ...
    VOICE_CHOICE = {"ic_male": "Мужской голос", "ic_female": "Женский голос", "ic_custom": "Установить свой вариант"}
    ...
    MODEL = VoiceMatica()
    DEFAULT_VOICE = "prompts_ru/female.waw"
    DEFAULT_TEXT = "Hello world!"
    VOICE_CHOICE = {"ic_male": "Male voice", "ic_female": "Female voice", "ic_custom": "Set your voice variant"}
    BUILTIN_VOICE_CHOICE = "{} has been successfully installed.\n" \
                        "It will be used for further voicing.\n" \
                        "Example of sounding the phrase 'Hello world!':"
    CUSTOM_VOICE_CHOICE = "You have chosen your variant.\n" \
                        "Record a voice message with the desired voice for at least 3sec. " \
                        "It should be recorded in English to improve quality."
    FAIL_VOICE_SETUP = "I can't use this massage as voice prompt.\n" \
                    "The voice setup process is reset."
    LONG_VOICE_SETUP = "The audio must be at least 3 seconds long.\n" \
                    "The voice setup process is reset."
    NEXT_MSG_FORWARDED_FROM = "Next message forwarded from {}.\n"
    EMPTY_MESSAGE = "I can't voice the message without text"
    ...
    if chat_id in chat_to_is_setup:
        ...
        if ("message" in message) and ("voice" in message["message"]):
            if message["message"]["voice"]["duration"] > 3:
                ...
                extension = message["message"]["voice"]["mime_type"].split("/")[-1]
                ...
                tel_send_message(chat_id, BUILTIN_VOICE_CHOICE.format("custom"))
                txt = DEFAULT_TEXT
                succeed = True
            else:
    ...
    if request.method == 'POST':
        ...
        if is_inline_buttom:
            ...
            if voice in ["male", "female"]:
                ...
                tel_send_message(chat_id, BUILTIN_VOICE_CHOICE.format(VOICE_CHOICE[txt]))
...
def tel_get_file(file_id, path):
    response = requests.get(f"https://api.telegram.org/bot{TOKEN}/getFile?file_id={file_id}").json()
    file_path = response["result"]["file_path"]
    audio_file = requests.get(f"https://api.telegram.org/file/bot{TOKEN}/{file_path}")
